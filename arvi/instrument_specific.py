...
class ISSUES:
    ...
    def known_issues_w_record(self, mask=True, plot=False, df=None, **kwargs):
            """ Identify and optionally mask known instrumental issues.
            ...
            try:
                results = np.logical_or.reduce(results)
                removed_points = self.raw_file[results]
                pd_header = 'raw_file'

                if len(removed_points) > 0:
                    new_df = pd.DataFrame({pd_header: removed_points, 'reason': ['known_issue'] * len(removed_points)})
                    if df is not None:
